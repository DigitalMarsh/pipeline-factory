...
export const api = {
  ...
  discard: (planId: string) => request<{ plan: Plan }>(`/api/v3/plans/${planId}/discard`, { method: "POST", body: JSON.stringify({ actorId: "local-user" }) }),
  ...
  cancelRun: (runId: string, reason = "user_requested") => request<{ run: Run }>(`/api/v3/runs/${runId}/cancel`, { method: "POST", body: JSON.stringify({ reason }) }),
