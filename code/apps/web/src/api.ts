...
export const api = {
  ...
  explorerActivity: (projectId: string, explorerId: string, explorerPlanId?: string) => request<{ items: ExplorerActivityItem[]; lastEventSequence: number }>(`/api/v4/projects/${projectId}/explorers/${encodeURIComponent(explorerId)}/activity${explorerPlanId ? `?explorerPlanId=${encodeURIComponent(explorerPlanId)}` : ""}`),
  ...
  startExplorerTurn: (projectId: string, threadId: string, content: string, clientTurnId: string, explorerPlanId?: string) => request<{ turn: { user: ExplorerTurn; assistant: ExplorerTurn }; eventsUrl: string; loopId: string | null; state: string }>(`/api/v4/projects/${projectId}/explorer-thread/turns`, { method: "POST", body: JSON.stringify({ threadId, content, clientTurnId, ...(explorerPlanId ? { explorerPlanId } : {}) }) }),
  getExplorerTurns: (projectId: string, threadId: string, explorerPlanId?: string) => request<{ items: ExplorerTurn[]; lastEventSequence: number }>(`/api/v4/projects/${projectId}/explorer-thread/turns?threadId=${encodeURIComponent(threadId)}${explorerPlanId ? `&explorerPlanId=${encodeURIComponent(explorerPlanId)}` : ""}`),
  explorerAgentLoops: (projectId: string, threadId: string, explorerPlanId?: string) => request<{ items: AgentLoop[] }>(`/api/v4/projects/${projectId}/explorer-thread/agent-loops?threadId=${encodeURIComponent(threadId)}${explorerPlanId ? `&explorerPlanId=${encodeURIComponent(explorerPlanId)}` : ""}`),
  inputRequests: (projectId: string, threadId: string, status?: string, explorerPlanId?: string) => request<{ items: ExplorerInputRequest[] }>(`/api/v4/projects/${projectId}/explorer-thread/input-requests?threadId=${encodeURIComponent(threadId)}${status ? `&status=${encodeURIComponent(status)}` : ""}${explorerPlanId ? `&explorerPlanId=${encodeURIComponent(explorerPlanId)}` : ""}`),
