...
const form = reactive({
  ...
  maxParallelRuns: 2, defaultTimeoutMs: 120000, maxAutoContinuationTurns: 4, maxRepairAttempts: 2,
  ...
  startEnabled: false, startCommandId: "", startTimeoutMs: 120000, cleanupEnabled: false, cleanupCommandId: "", cleanupTimeoutMs: 120000,
...
function setForm(value: Project) {
  ...
  Object.assign(form, {
    ...
    maxParallelRuns: settings.concurrency.maxParallelRuns, defaultTimeoutMs: settings.concurrency.defaultTimeoutMs, maxAutoContinuationTurns: settings.concurrency.maxAutoContinuationTurns, maxRepairAttempts: settings.concurrency.maxRepairAttempts,
    ...
    startEnabled: settings.hooks.start?.enabled !== false && Boolean(settings.hooks.start), startCommandId: settings.hooks.start?.commandId ?? "", startTimeoutMs: settings.hooks.start?.timeoutMs ?? 120000,
    cleanupEnabled: settings.hooks.cleanup?.enabled !== false && Boolean(settings.hooks.cleanup), cleanupCommandId: settings.hooks.cleanup?.commandId ?? "", cleanupTimeoutMs: settings.hooks.cleanup?.timeoutMs ?? 120000,
...
function settingsPayload(): ProjectSettings {
  const lifecycle = {
    ...
    ...(form.cleanupCommandId ? { cleanup: { commandId: form.cleanupCommandId, enabled: form.cleanupEnabled, timeoutMs: form.cleanupTimeoutMs } } : {}),
  };
  return {
    concurrency: { maxParallelRuns: form.maxParallelRuns, defaultTimeoutMs: form.defaultTimeoutMs, maxAutoContinuationTurns: form.maxAutoContinuationTurns, maxRepairAttempts: form.maxRepairAttempts },
  ...
  <div class="project-settings-page" v-loading="loading">
    ...
    <div v-if="project" class="settings-shell">
      ...
      <main class="settings-content">
        ...
        <section v-else-if="activeTab === 'execution'" class="settings-section"><div class="section-title"><div><h2>Execution policy</h2><p>这些设置会在下一次 Plan Confirm 时冻结。</p></div></div><div class="settings-form-grid"><label>Max parallel runs<input v-model.number="form.maxParallelRuns" :disabled="project.status === 'ARCHIVED'" type="number" min="1" max="32" /></label><label>Default timeout (ms)<input v-model.number="form.defaultTimeoutMs" :disabled="project.status === 'ARCHIVED'" type="number" min="1000" /></label><label>Auto continuation turns<input v-model.number="form.maxAutoContinuationTurns" :disabled="project.status === 'ARCHIVED'" type="number" min="0" max="20" /></label><label>Max repair attempts<input v-model.number="form.maxRepairAttempts" :disabled="project.status === 'ARCHIVED'" type="number" min="0" max="20" /></label></div><div class="settings-notice"><InfoFilled :size="16" /><div><strong>快照策略</strong><p>已确认 Plan 和运行中的 Run 使用自己的 Project 配置快照；修改这里不会改变历史执行。</p></div></div></section>
        ...
        <section v-else-if="activeTab === 'hooks'" class="settings-section"><div class="section-title"><div><h2>Lifecycle hooks</h2><p>Start 在 Worktree 创建后执行，Cleanup 在 Worktree 移除后执行。</p></div></div><div class="hook-editor"><div><h3>Start hook</h3><p>初始化新的 Run Worktree。</p></div><el-switch v-model="form.startEnabled" :disabled="project.status === 'ARCHIVED'" /><div class="settings-form-grid"><label>Command ID<input v-model="form.startCommandId" :disabled="project.status === 'ARCHIVED'" placeholder="project.start" /></label><label>Timeout (ms)<input v-model.number="form.startTimeoutMs" :disabled="project.status === 'ARCHIVED'" type="number" min="1000" /></label></div></div><div class="hook-editor"><div><h3>Cleanup hook</h3><p>从稳定 Project 目录执行清理。</p></div><el-switch v-model="form.cleanupEnabled" :disabled="project.status === 'ARCHIVED'" /><div class="settings-form-grid"><label>Command ID<input v-model="form.cleanupCommandId" :disabled="project.status === 'ARCHIVED'" placeholder="project.cleanup" /></label><label>Timeout (ms)<input v-model.number="form.cleanupTimeoutMs" :disabled="project.status === 'ARCHIVED'" type="number" min="1000" /></label></div></div></section>
