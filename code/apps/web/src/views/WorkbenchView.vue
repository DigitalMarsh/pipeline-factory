...
function openProject(project: { id: string }) {
  void router.push("/projects/" + project.id + "/plans");
}
    ...
    <div class="workbench-layout" v-loading="loading">
      ...
      <main class="plan-inspector">
        <div v-if="selectedPlan" class="inspector-content">
          ...
          <section class="inspector-section"><div class="inspector-section-heading"><span>03</span><strong>Scope & verification</strong></div><div class="inspector-scope-grid"><div><label>INCLUDE</label><code v-for="item in selectedPlan.contract.include" :key="item">{{ item }}</code></div><div><label>VERIFY</label><code v-for="item in selectedPlan.contract.verificationCommandIds" :key="item">{{ item }}</code></div></div></section>
...
<style scoped>
.workbench-page { min-height: calc(100vh - 60px); padding: 30px 38px 40px; background: #f7f9fc; color: #1e2b3d; }
...
.workbench-context { min-width: 0; padding: 18px 15px; border-left: 1px solid #e8edf3; background: #fbfcfe; }.context-ok { color: #42ad7c; }.context-block { padding: 19px 4px 0; }.context-block + .context-block { margin-top: 18px; padding-top: 18px; border-top: 1px solid #e8edf3; }.context-block-title { display: flex; align-items: center; gap: 7px; color: #63738a; font-size: 10px; font-weight: 800; }.context-block-title svg { color: #7796dd; }.context-block-title small { margin-left: auto; color: #a6b0bd; font-size: 9px; font-weight: 400; }.context-facts { display: grid; gap: 10px; margin-top: 13px; }.context-facts span { margin-bottom: 4px; letter-spacing: .04em; }.context-facts code { display: block; overflow: hidden; color: #6178a6; text-overflow: ellipsis; white-space: nowrap; }.context-muted { margin-top: 10px; color: #a3aebb; font-size: 9px; line-height: 1.5; }.memory-card { display: grid; gap: 7px; margin-top: 11px; padding: 11px; border: 1px solid #e2e9f2; border-radius: 7px; background: #fff; }.memory-card strong { color: #5c6c84; font-size: 10px; }.memory-card span { color: #9ba6b5; font-size: 9px; }.evidence-list { display: grid; gap: 10px; margin-top: 13px; }.evidence-item { display: flex; align-items: flex-start; gap: 7px; }.evidence-dot { width: 6px; height: 6px; flex: 0 0 6px; margin-top: 4px; border-radius: 50%; background: #d5a33b; }.evidence-dot.success { background: #43b27e; }.evidence-item strong, .evidence-item small { display: block; }.evidence-item strong { color: #697991; font-size: 9px; }.evidence-item small { margin-top: 3px; color: #a8b1bd; font-size: 8px; }.context-footer { display: flex; flex-wrap: wrap; align-items: center; gap: 5px; margin: 26px 4px 0; padding-top: 13px; border-top: 1px solid #e8edf3; color: #a0abb8; font-size: 9px; }.context-footer a { display: flex; align-items: center; gap: 4px; margin-left: auto; color: #5d7ed2; }.context-footer .live-dot { width: 5px; height: 5px; background: #42b27c; }.workbench-empty { display: grid; justify-items: center; gap: 7px; color: #a9b3c0; text-align: center; }.workbench-empty strong { color: #64748a; font-size: 11px; }.workbench-empty span { max-width: 180px; font-size: 9px; line-height: 1.5; }.workbench-empty a { color: #537bd7; font-size: 9px; }.workbench-empty.compact { margin-top: 28px; padding: 22px 10px; border: 1px dashed #dce4ee; border-radius: 7px; }.workbench-empty.inspector-empty { height: 100%; align-content: center; }
@media (max-width: 1120px) { .workbench-page { padding: 25px 20px; }.workbench-layout { grid-template-columns: 230px minmax(420px, 1fr) 240px; }.plan-inspector { padding-left: 25px; padding-right: 25px; } }
</style>
