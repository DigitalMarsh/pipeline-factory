...
type ProjectSettingsPatch = Omit<ProjectSettings, "concurrency"> & { concurrency: Omit<ProjectSettings["concurrency"], "maxParallelRuns" | "maxAutoContinuationTurns"> };
...
const form = reactive({
  ...
  defaultTimeoutMs: 120000, executionTimeoutMs: 1800000, maxRepairAttempts: 2,
...
function setForm(value: Project) {
  ...
  Object.assign(form, {
    ...
    defaultTimeoutMs: settings.concurrency.defaultTimeoutMs, executionTimeoutMs: settings.concurrency.executionTimeoutMs ?? 1800000, maxRepairAttempts: settings.concurrency.maxRepairAttempts,
...
function settingsPayload(): ProjectSettingsPatch {
  ...
  return {
    concurrency: { defaultTimeoutMs: form.defaultTimeoutMs, executionTimeoutMs: form.executionTimeoutMs, maxRepairAttempts: form.maxRepairAttempts },
    ...
    <div class="settings-dialog-body" v-loading="loading">
      ...
      <div v-if="project" class="settings-shell">
        ...
        <main class="settings-content">
          ...
          <section v-else-if="activeTab === 'execution'" class="settings-section"><div class="section-title"><div><h2>Execution policy</h2><p>这些设置会在下一次 Plan Confirm 时冻结。</p></div></div><div class="settings-form-grid"><label>Default timeout (ms)<input v-model.number="form.defaultTimeoutMs" :disabled="project.status === 'ARCHIVED'" type="number" min="1000" /></label><label>Execution timeout (ms)<input v-model.number="form.executionTimeoutMs" :disabled="project.status === 'ARCHIVED'" type="number" min="1000" /></label><label>Max repair attempts<input v-model.number="form.maxRepairAttempts" :disabled="project.status === 'ARCHIVED'" type="number" min="0" max="20" /></label></div><div class="settings-notice"><InfoFilled :size="16" /><div><strong>快照策略</strong><p>已确认 Plan 和运行中的 Run 使用自己的 Project 配置快照；修改这里不会改变历史执行。</p></div></div></section>
