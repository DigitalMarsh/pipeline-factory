  ...
  <section class="plan-center-panel" aria-label="计划中心">
    ...
    <div v-loading="loading" class="plan-center-list">
      <article v-for="plan in filtered" :key="plan.planId ?? plan.id ?? plan.title" class="plan-center-card">
        ...
        <div class="plan-center-card-meta"><span>Run</span><button v-if="plan.runId" type="button" class="plan-center-run-link" @click="emit('open-run', plan)">{{ plan.runId }} <ArrowRight :size="12" /></button><span v-else>{{ plan.status === "ENQUEUED" ? "Ready to start" : plan.dispatch?.waitReason === "NEEDS_CONFIGURATION" ? "Needs configuration" : plan.dispatch?.waitReason ?? "—" }}</span></div>
