...
<template>
  <aside class="thread-rail">
    <div class="rail-project">
      ...
      <div><strong>project-demo</strong><small>Local workspace</small></div>
      <span class="chevron">⌄</span>
    </div>
    <div class="rail-label">EXPLORER THREAD</div>
    <div class="thread-identity">
      <div class="thread-icon"><Connection :size="16" /></div>
      <div class="thread-copy"><strong>Product workspace</strong><small>{{ thread?.id ?? "thread-demo" }}</small></div>
      <span class="live-dot" />
    </div>
    ...
    <div class="rail-section">
      ...
      <RouterLink class="rail-link subdued" :to="`/projects/${thread?.projectId ?? 'project-demo'}/explorer#successors`"><Connection :size="15" /> Successor threads <span>›</span></RouterLink>
      <RouterLink class="rail-link subdued" :to="`/projects/${thread?.projectId ?? 'project-demo'}/explorer#summary`"><Files :size="15" /> Context summary <span>›</span></RouterLink>
    </div>
